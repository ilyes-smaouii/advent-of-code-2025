...
std::pair<std::vector<button_t_v3>, joltages_t>
parseLine_p2_v3(const std::string &line) {
  auto res = std::make_pair<std::vector<button_t_v3>, joltages_t>({}, {});
  auto &[buttons, goal_jolts] = res;
  buttons.reserve(MAX_BUTTON_COUNT);
  goal_jolts.reserve(MAX_LIGHT_COUNT);
  auto line_iter = line.begin();
...
struct NodeLess {
  bool operator()(const node_t &a, const node_t &b) const {
    ...
    // }
    // const auto a_score = a_presses_sum + a_est, b_score = b_presses_sum +
    // b_est;
    [[unlikely]] if (a_est == b_est) {
      return std::lexicographical_compare(
          a_press_counts.begin(), a_press_counts.end(), b_press_counts.begin(),
          b_press_counts.end());
    } else {
      return std::less()(a_est, b_est);
    }
  }
};
...
std::pair<button_t, count_t> findBiggestButton(const buttons_t &buttons) {
  count_t biggest_button_size{0};
  button_t biggest_button{};
  for (const auto &button : buttons) {
    auto curr_size = button.size();
    if (curr_size > biggest_button_size) {
      biggest_button_size = curr_size;
      biggest_button = button;
    }
  }
  return {biggest_button, biggest_button_size};
}

small_count_t findButtonSize_v3(const button_t_v3 &button) {
...
std::pair<button_t_v3, count_t>
findBiggestButton_v3(const buttons_t_v3 &buttons) {
  count_t biggest_button_size{0};
  button_t_v3 biggest_button{};
  for (const auto &button : buttons) {
    auto curr_size = findButtonSize_v3(button);
    if (curr_size > biggest_button_size) {
      biggest_button_size = curr_size;
      biggest_button = button;
    }
...
joltages_t tryButtonPresses(const buttons_t &buttons,
  ...
  auto res = joltages_t(buttons.size(), 0);
...
joltages_t tryButtonPresses_v3(const buttons_t_v3 &buttons,
                               const press_counts_t &press_counts) {
  // joltages_t res{buttons.size(), static_cast<joltages_t::value_type>(0)};
  auto res = joltages_t(buttons.size(), 0);
  if (buttons.size() != press_counts.size()) {
  ...
  for (std::size_t i{0}; i < buttons.size(); i++) {
    button_t_v3 curr_but{buttons.at(i)};
    std::size_t light_idx{0};
    while (curr_but > 0) {
      res[light_idx] += (curr_but & 1);
      curr_but >>= 1;
    }
    // for (const auto &light_idx : buttons[i]) {
    //   res[light_idx] += press_counts[i];
...
count_t sumButtonComponents(const button_t &button) {
  return std::accumulate(button.begin(), button.end(), 0, std::plus());
}

// count_t sumButtonComponents_v3(const button_t &button) {
//   return std::accumulate(button.begin(), button.end(), 0, std::plus());
// }

count_t sumPresses(const press_counts_t &presses) {
  return std::accumulate(presses.begin(), presses.end(), 0, std::plus());
}
...
node_t findMinButtonPresses(const buttons_t &buttons,
  ...
  //
  auto initial_candidate = std::tuple(initial_press_counts, 0,
                                      initial_jolt_levels, 0, initial_estimate);
  //
  std::set<node_t, NodeLess> candidates{initial_candidate};
  ...
  while (candidates.size() > 0) {
    ...
    const auto &best_candidate = *(candidates.begin());
    // std::cout << "[candidate] Best candidate : "
    //           << hlp::vectorToString(std::get<PRESSES>(best_candidate))
    //           << std::endl; // [debugging]
    // auto attempt = tryButtonPresses(buttons,
    // std::get<PRESSES>(best_candidate));
    const auto &attempt = std::get<JOLT_LVLS>(best_candidate);
    if (std::equal(attempt.begin(), attempt.end(), goal_joltages.begin(),
                   goal_joltages.end())) {
      std::cout << "returning, iter count : " << iter_count
                << std::endl; // [debugging]
      ...
      // }
      explored.insert(*candidates.begin());
      candidates.erase(candidates.begin());
      // std::cout
      //     << "Removed best candidate from candidates and added it to explored
      //     !"
      //     << std::endl; // [debugging]
      ...
      for (std::size_t i{0}; i < button_count; i++) {
        ...
        auto curr_neighbor = candidate_copy;
        {
          ...
          std::get<PRESSES_SUM>(curr_neighbor) += 1; // adding to presses sum
          for (const auto &light_idx : buttons[i]) {
        ...
        bool is_valid_candidate{true},
            has_been_explored{explored.contains(curr_neighbor)};
        if (!has_been_explored) {
          ...
          // std::cout << "neighbor's validity as candidate : " <<
          // std::boolalpha
  ...
  std::cout << "loop finished, iter count : " << iter_count
            << std::endl; // [debugging]
  throw std::runtime_error(
      "findMinButtonPresses() error : didn't find solution !");
}

node_t findMinButtonPresses_v3(const buttons_t_v3 &buttons,
                               const joltages_t &goal_joltages) {
  const count_t light_count{goal_joltages.size()};
  const count_t button_count{buttons.size()};
  auto initial_press_counts = press_counts_t(button_count, 0);
  auto initial_jolt_levels = joltages_t(light_count, 0);
  auto [biggest_button, biggest_button_size] = findBiggestButton_v3(buttons);
  count_t goal_jolt_sum = std::accumulate(
      goal_joltages.begin(), goal_joltages.end(), 0, std::plus<joltage_t>());
  count_t initial_estimate = goal_jolt_sum / biggest_button_size;
  //
  ...
  //
  std::set<node_t, NodeLess> candidates{initial_candidate};
  std::unordered_set<press_counts_t, VecHash<press_counts_t::value_type>>
      explored{};
  // explored.reserve(explored.size() + 1'000'000);
  //
  std::size_t iter_count{0};
  std::cout << "about to enter loop" << std::endl; // [debugging]
  while (candidates.size() > 0) {
    ...
    if (std::equal(attempt.begin(), attempt.end(), goal_joltages.begin(),
      ...
      //   std::cout << "Removed best candidate !" << std::endl; // [debugging]
      // }
      explored.insert(std::get<PRESSES>(*candidates.begin()));
      candidates.erase(candidates.begin());
      // std::cout
      //     << "Removed best candidate from candidates and added it to explored
      //     !"
      ...
      for (std::size_t i{0}; i < button_count; i++) {
        ...
        auto curr_neighbor = candidate_copy;
        {
          std::get<PRESSES>(curr_neighbor)[i] +=
              1; // adding relevant button press
          std::get<PRESSES_SUM>(curr_neighbor) += 1; // adding to presses sum
          auto curr_button{buttons[i]};
          for (std::size_t light_idx{0}; light_idx < light_count; light_idx++) {
            std::get<JOLT_LVLS>(curr_neighbor)[light_idx] += (curr_button & 1);
            std::get<JOLTS_SUM>(curr_neighbor) += (curr_button & 1);
            curr_button >>= 1;
          }
          std::get<EST>(curr_neighbor) =
              (std::get<PRESSES_SUM>(curr_neighbor) >> 1) +
              ((goal_jolt_sum - std::get<JOLTS_SUM>(curr_neighbor)) /
               biggest_button_size);
        }
        // std::cout << "set up neighbor n°" << i << std::endl; // [debugging]
        bool is_valid_candidate{true}, has_been_explored{explored.contains(
                                           std::get<PRESSES>(curr_neighbor))};
        for (std::size_t i{0}; i < light_count; i++) {
          {
            // if (std::get<JOLT_LVLS>(curr_neighbor)[i] > goal_joltages[i]) {
            //   is_valid_candidate = false;
          ...
          {
            is_valid_candidate &=
                (std::get<JOLT_LVLS>(curr_neighbor)[i] <= goal_joltages[i]);
          }
        }
        // std::cout << "neighbor's validity as candidate : " <<
        // std::boolalpha
        //           << is_valid_candidate << std::endl; // [debugging]
        if (is_valid_candidate & !has_been_explored) {
          candidates.insert(curr_neighbor);
          // std::cout << "Just added a new candidate to explore"
          //           << std::endl;                        // [debugging]
          // std::cout << "neighbor inserted" << std::endl; // [debugging]
...
[.###.#] (0,1,2,3,4) (0,3,4) (0,1,2,4,5) (1,2) {10,11,11,5,10,5})");
  ...
  auto [buttons, goal_jolts] = parseLine_p2(le_lines[std::stoi(argv[1])]);
  auto buttons_v3 = parseLine_p2_v3(le_lines[std::stoi(argv[1])]).first;
  ...
  auto res = findMinButtonPresses_v3(buttons_v3, goal_jolts);
  hlp::printLogEntries(
      {"R"}, "Best candidate :", std::get<PRESSES>(res),
      std::format("(sum of presses : {})", sumPresses(std::get<PRESSES>(res))));
